...
use bitvmx_emulator_job::messages::{EmulatorJob, EmulatorJobType, EmulatorResultType};
use config;
...
fn main() -> Result<(), anyhow::Error> {
    ...
    let _verifier_result = EmulatorResultType::from_value(verifier_result)?.as_verifier_check()?;
    ...
    for round in 1..total_rounds + 1 {
        ...
        let hashes = EmulatorResultType::from_value(prover_hashes_result)?.as_prover_hashes()?;
